...
export class MAIFBinaryViewerProvider implements vscode.CustomReadonlyEditorProvider {
    public static readonly viewType = 'maif.binaryViewer';
    ...
    private detectFormat(data: Uint8Array): FileInfo {
        if (data.length < 4) {
    ...
    private parseBlocks(data: Uint8Array, fileInfo: FileInfo): BlockInfo[] {
    ...
    private parseSecureBlocks(data: Uint8Array, blockCount: number): BlockInfo[] {
        ...
        for (let i = 0; i < blockCount && offset + MAIFBinaryViewerProvider.SECURE_BLOCK_HEADER_SIZE <= data.length; i++) {
            ...
            const type = String.fromCharCode(
                (blockTypeCode >> 24) & 0xFF,
                (blockTypeCode >> 16) & 0xFF,
                (blockTypeCode >> 8) & 0xFF,
                blockTypeCode & 0xFF
            );
            ...
            let contentType: 'text' | 'json' | 'binary' = 'binary';
            ...
            offset += size;
            if (blocks.length >= 100) break; // Safety limit
        }

        return blocks;
    }

    private generateBlockHexPreview(data: Uint8Array, maxBytes: number = 256): string {
    ...
    private getBlockTypeName(type: string): string {
    ...
    private getBlockTypeClass(type: string): string {
    ...
    private parseLegacyBlocks(data: Uint8Array): BlockInfo[] {
        const blocks: BlockInfo[] = [];
        let offset = 0;
    ...
    private generateHexDump(data: Uint8Array, maxLines: number = 128): string[] {
        const lines: string[] = [];
    ...
    private truncateHash(hash: string, len: number = 16): string {
        if (hash.length <= len * 2) return hash;
        return `${hash.slice(0, len)}...${hash.slice(-8)}`;
    }

    private getHtmlContent(filename: string, totalBytes: number, hexLines: string[], blocks: BlockInfo[], fileInfo: FileInfo): string {
