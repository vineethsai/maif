...
export interface MAIFProvenanceEntry {
    timestamp: number;
    agent_id: string;
    agent_did?: string;
    action: string;
...
export interface MAIFManifest {
    maif_version: string;
    format?: 'legacy' | 'secure';
    created: number;
    modified?: number;
    agent_id?: string;
    creator_id?: string;
    ...
    block_registry?: Record<string, MAIFBlock[]>;
    root_hash?: string;
    merkle_root?: string;
    signature?: string;
    public_key?: string;
    signature_metadata?: {
        ...
        provenance_chain: MAIFProvenanceEntry[];
    };
}

export interface MAIFFileInfo {
    format: 'legacy' | 'secure';
    version: string;
    flags: number;
    created: number;
...
export interface MAIFSecurityInfo {
    publicKey?: string;
    signerId?: string;
    signerDid?: string;
    signedAt?: number;
...
export class MAIFParser {
    manifest: MAIFManifest | null = null;
    binaryData: Uint8Array | null = null;
    blocks: MAIFBlock[] = [];
    provenance: MAIFProvenanceEntry[] = [];
    fileInfo: MAIFFileInfo | null = null;
    securityInfo: MAIFSecurityInfo | null = null;
    format: 'legacy' | 'secure' | 'unknown' = 'unknown';
    ...
    static BLOCK_TYPES: Record<string, BlockTypeInfo> = {
    ...
    loadManifestFromString(content: string): void {
        try {
    ...
    private detectAndParseBinary(): void {
    ...
    private parseSecureBinary(): void {
    ...
    private parseSecureBlock(offset: number, index: number): MAIFBlock | null {
    ...
    private parseProvenanceSection(offset: number): void {
    ...
    private parseSecuritySection(offset: number): void {
        if (!this.binaryData || offset === 0 || offset >= this.binaryData.length) {
    ...
    private buildCompatibilityManifest(): void {
        this.manifest = {
    ...
    private parseLegacyBinary(): void {
        if (!this.binaryData) return;

        if (this.blocks.length > 0) return;

        const parsedBlocks: MAIFBlock[] = [];
        let offset = 0;
    ...
    getBlockData(blockIndex: number): Uint8Array | null {
    ...
    getTextContent(blockIndex: number): string | null {
    ...
    getBlockTypeInfo(type: string): BlockTypeInfo {
        return MAIFParser.BLOCK_TYPES[type] || { name: type, icon: '?', color: '#64748b' };
    }

    getHexDump(start: number = 0, length: number = 256): Array<{
        offset: string;
        hex: string;
        ascii: string;
    }> {
    ...
    private nullTerminatedString(bytes: Uint8Array): string {
    ...
    private blockTypeCodeToString(code: number): string {
        const chars = [
            (code >> 24) & 0xFF,
            (code >> 16) & 0xFF,
            (code >> 8) & 0xFF,
