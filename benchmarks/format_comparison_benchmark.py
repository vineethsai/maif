...
class FormatComparisonBenchmark:
    ...
    def _benchmark_file_creation(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            for size in self.text_sizes:
                ...
                print(f"  Size {size:,} bytes: Legacy {statistics.mean(legacy_times_for_size)*1000:.2f}ms, "
        ...
        self.metrics.append(BenchmarkMetrics(
        ...
        print(f"  ✓ Average: Legacy {avg_legacy:.2f}ms, Secure (Ed25519) {avg_secure:.2f}ms")
    ...
    def _benchmark_file_size(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            for size in self.text_sizes:
                for block_count in [5, 20]:
                    ...
                    # Secure format
                    secure_path = os.path.join(tmpdir, f"secure_{size}_{block_count}.maif")
                    ...
                    legacy_overhead = ((legacy_total - raw_data_size) / raw_data_size) * 100
                    secure_overhead = ((secure_total - raw_data_size) / raw_data_size) * 100
                    
                    print(f"  {block_count} blocks × {size:,}B: Legacy {legacy_total:,}B ({legacy_overhead:.1f}% overhead), "
        ...
        print(f"  ✓ Average overhead: Legacy {avg_legacy_overhead:.1f}%, Secure {avg_secure_overhead:.1f}%")
    ...
    def _benchmark_read_performance(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            for size in [1000, 10000, 50000]:
                ...
                print(f"  Size {size:,}: Legacy {statistics.mean(legacy_read_times)*1000:.2f}ms, "
    ...
    def _benchmark_integrity_verification(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            for block_count in [10, 50, 100]:
                ...
                print(f"  {block_count} blocks: Legacy {statistics.mean(legacy_verify_times)*1000:.2f}ms, "
    ...
    def _benchmark_tamper_detection(self):
        ...
        print(f"  ✓ Detection rate: Legacy {legacy_rate:.1f}%, Secure {secure_rate:.1f}%")
        print(f"  ✓ Detection time: Legacy {avg_legacy_time:.2f}ms, Secure {avg_secure_time:.2f}ms")
    ...
    def _benchmark_scalability(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            for block_count in [10, 50, 100, 200]:
                ...
                print(f"  {block_count} blocks: Legacy {legacy_create_time*1000:.1f}ms/{legacy_size/1024:.1f}KB, "
    ...
    def _benchmark_security_features(self):
        ...
        print(f"  ✓ Score: Legacy {legacy_score}/{max_score}, Secure {secure_score}/{max_score}")
