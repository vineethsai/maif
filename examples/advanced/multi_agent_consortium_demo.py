...
class BaseAgent:
    ...
    def __init__(
        self, agent_id: str, agent_type: str, specialization: str, shared_maif=None
    ):
        ...
        try:
            response = self.openai_client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": f"You are a {self.agent_type} agent specializing in {self.specialization}. Provide detailed, structured analysis in JSON format.",
                    },
    ...
    def contribute(
        ...
        base_confidence = 0.7
        iteration_bonus = min(
            0.2, self.iteration_count * 0.05
        )  # Max 0.2 bonus for iterations
    ...
    def refine_contribution(
        self, original_contribution: AgentContribution, feedback: Dict[str, Any]
    ) -> AgentContribution:
        ...
        self.refinement_history.append(
            {
                ...
                "confidence_change": refined_contribution.confidence
                - original_contribution.confidence,
            }
        )
    ...
    def _apply_refinements(
        self, original: AgentContribution, feedback: Dict[str, Any]
    ) -> AgentContribution:
    ...
    def add_contribution_to_maif(
        self, contribution: AgentContribution, update_existing: bool = False
    ) -> str:
        ...
        existing_block_id = (
            self.content_blocks.get(content_key) if update_existing else None
        )
        ...
        metadata = {
            ...
            "content_summary": str(contribution.content)[:200] + "..."
            if len(str(contribution.content)) > 200
            else str(contribution.content),
        }
        ...
        content_id = self.maif.encoder.add_binary_block(
            content_bytes, "text", metadata=metadata
        )
        ...
        if isinstance(contribution.content, dict) and "summary" in contribution.content:
            ...
            try:
                self.maif.add_embeddings(
                    embeddings, f"{self.agent_id}_embedder", compress=True
                )
            except Exception as e:
    ...
    def _calculate_complexity_score(self, content: Any) -> float:
        ...
        if isinstance(content, dict):
            return min(
                10.0,
                len(content) * 0.5
                + sum(len(str(v)) for v in content.values()) / 1000.0,
            )
    ...
    def _add_knowledge_triples(self, contribution: AgentContribution, content_id: str):
        ...
        try:
            ...
            print(
                f"Warning: Could not create knowledge triples for {self.agent_id}: {e}"
            )
...
class GeographyAgent(BaseAgent):
    ...
    def __init__(
        self,
        agent_id: str = "geo_agent_001",
        is_devils_advocate: bool = False,
        shared_maif=None,
    ):
        super().__init__(
            agent_id, "geography", "terrain_analysis_and_routing", shared_maif
        )
    ...
    def contribute(
        ...
        print(
            f"🔥 DEBUG: {self.agent_id} contributing (devil's advocate: {self.is_devils_advocate})"
        )
                    ...
                    "route_segments": [
                        {
                            ...
                            "challenges": [
                                "ocean_entry",
                                "CRITICAL: No safe ocean entry points",
                                "Legal restrictions on ocean access",
                            ],
                        },
                        {
                            ...
                            "terrain": "deep_ocean",
                            "challenges": [
                                "IMPOSSIBLE: Even infinite swimming faces storms",
                                "Hypothermia guaranteed",
                                "Navigation impossible without landmarks",
                                "International waters legal issues",
                            ],
                        },
                        {
                            ...
                            "challenges": [
                                "CRITICAL: Visa and border issues",
                        ...
                        },
                        {
                            ...
                            "challenges": [
                                "FATAL: Altitude sickness unavoidable",
                                "Avalanche zones",
                                "Permit requirements",
                            ],
                        },
        ...
        return AgentContribution(
            agent_id=self.agent_id,
            agent_type=self.agent_type,
            contribution_type="geographical_route_analysis"
            + ("_critical" if self.is_devils_advocate else ""),
            ...
            metadata={
                ...
                "role_modifier": "critical_analysis"
                if self.is_devils_advocate
...
class CulturalAgent(BaseAgent):
    ...
    def __init__(self, shared_maif=None):
        super().__init__(
            "culture_agent_001",
            "cultural",
            "cross_cultural_experience_design",
            shared_maif,
        )
...
class LogisticsAgent(BaseAgent):
    ...
    def __init__(self, shared_maif=None):
        super().__init__(
            "logistics_agent_001",
            "logistics",
            "resource_optimization_and_planning",
            shared_maif,
        )
        ...
        if self.use_openai:
            ...
            logistics_plan = {
                ...
                "timeline_optimization": {
                    ...
                    "key_segments": [
                        "Pacific crossing: 25-30 days",
                        "Land travel: 15-25 days",
                    ],
                },
...
class SafetyAgent(BaseAgent):
    ...
    def __init__(self, shared_maif=None):
        super().__init__(
            "safety_agent_001",
            "safety",
            "risk_mitigation_and_emergency_response",
            shared_maif,
        )
        ...
        if self.use_openai:
            ...
            safety_assessment = {
                "risk_categories": {
                    "environmental_risks": [
                        {
                            "risk": "Pacific Ocean hazards",
                            "severity": "extreme",
                            "mitigation": ["Weather monitoring", "Emergency protocols"],
                        },
                        {
                            "risk": "Himalayan altitude",
                            "severity": "high",
                            "mitigation": ["Acclimatization", "Oxygen backup"],
                        },
                    ],
                    "political_risks": [
                        {
...
class DevilsAdvocateAgent(BaseAgent):
    ...
    def __init__(self, shared_maif=None):
        super().__init__(
            "devils_advocate_001",
            "critical_analysis",
            "comprehensive_critique_and_challenge",
            shared_maif,
        )

    def contribute(
        self, query: str, other_contributions: Dict[str, AgentContribution] = None
    ) -> AgentContribution:
        ...
        return AgentContribution(
            ...
            dependencies=list(other_contributions.keys())
            if other_contributions
            else [],
            metadata={
...
class CollaborationStateMachine:
    ...
    def execute_collaboration_cycle(
        ...
        while (
            self.state != CollaborationState.COMPLETE
            ...
            if self.state == CollaborationState.INITIAL_CONTRIBUTIONS:
                current_contributions = self._state_collect_contributions(
                    other_agents, query
                )
                ...
                devils_advocate_feedback = self._state_devils_advocate_analysis(
                    devils_advocate_agent, current_contributions, query
                )
                ...
                targeted_feedback = self._state_generate_targeted_feedback(
                    devils_advocate_feedback, current_contributions
                )
                self._transition_to(CollaborationState.AGENT_REFINEMENT)

            elif self.state == CollaborationState.AGENT_REFINEMENT:
                current_contributions = self._state_refine_contributions(
                    other_agents, current_contributions, targeted_feedback
                )
                ...
                final_artifact = self._state_final_synthesis(
                    current_contributions, query
                )
                self._transition_to(CollaborationState.COMPLETE)
                print(
                    f"🔥 DEBUG: State machine returning {len(current_contributions)} contributions: {list(current_contributions.keys())}"
                )
    ...
    def _state_collect_contributions(
        ...
        for agent in agents:
            ...
            if previous_context:
                print(
                    f"    📚 Providing context from {len(previous_context.get('previous_rounds', 0))} previous rounds"
                )
    ...
    def _state_devils_advocate_analysis(
        self,
        devils_advocate: BaseAgent,
    ...
    def _extract_structured_feedback(
        self, da_contribution: AgentContribution
    ) -> Dict[str, Any]:
        ...
        if "agent_critique" in content:
            ...
            if isinstance(agent_critiques, dict):
                for critique_key, critique_value in agent_critiques.items():
                    ...
                    if (
                        "geography" in critique_key.lower()
                        or "geo" in critique_key.lower()
                    ):
                        agent_specific_feedback["geography"] = {
                            "critique": critique_value,
                            "action_items": self._generate_action_items(
                                critique_value, "geography"
                            ),
                            "priority": "high",
                        }
                    elif (
                        "cultural" in critique_key.lower()
                        or "culture" in critique_key.lower()
                    ):
                        agent_specific_feedback["cultural"] = {
                            "critique": critique_value,
                            "action_items": self._generate_action_items(
                                critique_value, "cultural"
                            ),
                            "priority": "high",
                        }
        ...
        return {
            ...
            "alternative_recommendations": content.get(
                "alternative_recommendations", []
            ),
        ...
        if isinstance(critique, str):
            ...
            if agent_type == "geography":
                if "distance" in critique_lower or "route" in critique_lower:
                    action_items.append(
                        "Recalculate distances using more accurate methods"
                    )
    ...
    def _state_generate_targeted_feedback(
        self,
        devils_advocate_feedback: Dict[str, Any],
        contributions: Dict[str, AgentContribution],
        ...
        for agent_id, contribution in contributions.items():
            ...
            if agent_type in agent_specific:
                ...
                feedback["suggestions"].extend(
                    [f"Action: {item}" for item in da_feedback["action_items"]]
                )
            ...
            critical_challenges = devils_advocate_feedback.get(
                "critical_challenges", []
            )
            if critical_challenges:
                relevant_challenges = [
                    c for c in critical_challenges[:2] if isinstance(c, str)
                ]
                feedback["suggestions"].extend(
                    [f"Address challenge: {c}" for c in relevant_challenges]
                )
        ...
        for agent in agents:
            if agent.agent_id in contributions and agent.agent_id in feedback:
                ...
                if agent_feedback.get("needs_refinement", False):
                    ...
                    content_id = agent.add_contribution_to_maif(
                        refined, update_existing=True
                    )
                    agent.maif.add_text(
...
class CoordinatorAgent(BaseAgent):
    ...
    def collect_contributions(
        ...
        final_contributions = self.state_machine.execute_collaboration_cycle(
            agents, query
        )
    ...
    def collect_contributions_legacy(
        self, agents: List[BaseAgent], query: str, enable_refinement: bool = True
    ) -> Dict[str, AgentContribution]:
        ...
        if hasattr(self, "agent_contributions") and self.agent_contributions:
            print(
                f"   📚 Building on {len(self.agent_contributions)} contributions from previous round"
            )
            ...
            ):  # More than just previous_round_contributions
                print(
                    f"   📚 Enhanced with rich MAIF context from {len([k for k in context.keys() if k != 'previous_round_contributions'])} agents"
                )
        elif context:
            print(
        ...
        for agent in agents:
            ...
            agent_rich_context = self._extract_rich_context_from_maif(
                agent, contribution
            )
        ...
        if enable_refinement:
            contributions = self._perform_iterative_refinement(
                agents, contributions, query
            )
    ...
    def _generate_cross_agent_feedback(
        ...
        for agent_id, contribution in contributions.items():
            ...
            other_contributions = {
                k: v for k, v in contributions.items() if k != agent_id
            }

            feedback = {
                "needs_refinement": len(other_contributions)
                > 0,  # Always refine if there are other contributions
        ...
        for agent in agents:
            try:
                ...
                if os.path.exists(agent_maif_path):
                    ...
                    agent_context = self._extract_context_from_loaded_maif(
                        loaded_maif, agent.agent_id
                    )
                    if agent_context:
    ...
    def _extract_context_from_loaded_maif(
        self, maif_artifact: MAIF, agent_id: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                hasattr(maif_artifact.decoder, "blocks")
                and maif_artifact.decoder.blocks
            ):
                for block in maif_artifact.decoder.blocks:
                    ...
                    if (
                        ...
                        and not block.metadata.get("encrypted", False)
                        if block.metadata
                        else True
                    ):
                        try:
                            content = maif_artifact.decoder.get_text_block(
                                block.block_id
                            )
                            block_summary["content_preview"] = (
                                content  # Show full content without truncation
                            )
                    ...
                    for i, embedding_data in enumerate(embeddings_list):
                        ...
                        if isinstance(embedding_data, dict):
                            context["embeddings_info"][model_name] = {
                                ...
                                "dimensions": len(
                                    embedding_data.get("vectors", [{}])[0]
                                )
                                if embedding_data.get("vectors")
                                else 0,
                            }
                            ...
                            context["embeddings_info"][model_name] = {
                                ...
                                "dimensions": len(embedding_data[0])
                                if embedding_data
                                and isinstance(embedding_data[0], list)
                                else 0,
                            }
            except Exception as e:
            ...
            for block in maif_artifact.decoder.blocks:
                if block.block_type == "cross_modal" and block.metadata:
                    context["cross_modal_data"][block.block_id] = {
                        ...
                        "unified_dim": block.metadata.get(
                            "unified_representation_dim", 0
                        ),
                    }

            # Extract version history if available
            if (
                hasattr(maif_artifact.decoder, "version_history")
                and maif_artifact.decoder.version_history
            ):
                context["version_history"] = {
                    block_id: len(versions)
                    for block_id, versions in maif_artifact.decoder.version_history.items()
                }
    ...
    def _extract_rich_context_from_maif(
        self, agent: BaseAgent, contribution: AgentContribution
    ) -> Dict[str, Any]:
        ...
        context = {
            ...
            "contribution_summary": {
                "contribution_type": contribution.contribution_type,
                "confidence": contribution.confidence,
                "content_keys": list(contribution.content.keys())
        ...
        try:
            if hasattr(agent.maif, "decoder"):
                ...
                context["embeddings_available"] = (
                    len(embeddings_list) if embeddings_list else 0
                )
        ...
        if hasattr(agent.maif, "decoder") and hasattr(agent.maif.decoder, "blocks"):
            cross_modal_count = sum(
                1
                for block in agent.maif.decoder.blocks
                if block.block_type == "cross_modal"
            )
            context["cross_modal_blocks"] = cross_modal_count

        # Add version tracking info
        if hasattr(agent.maif, "encoder") and hasattr(
            agent.maif.encoder, "version_history"
        ):
            context["version_tracking"] = {
                "tracked_blocks": len(agent.maif.encoder.version_history),
                "total_versions": sum(
                    len(versions)
                    for versions in agent.maif.encoder.version_history.values()
                ),
            }
            ...
            if hasattr(agent.maif.decoder, "blocks"):
                for block in agent.maif.decoder.blocks:
                    if (
                        ...
                        and block.metadata
                        and block.metadata.get("type") == "knowledge_triple"
                    ):
    ...
    def _generate_llm_summary_of_rounds(self, all_rounds_data: list, query: str) -> str:
        ...
        for round_data in all_rounds_data:
            ...
            for agent_id, contrib_data in round_data["contributions"].items():
                ...
                summary_prompt += (
                    f"  Content: {contrib_data['content_summary'][:200]}...\n"
                )
            ...
            try:
                ...
                response = openai.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": "You are an expert analyst specializing in multi-agent collaboration and consortium dynamics.",
                        },
    ...
    def _generate_fallback_summary(self, all_rounds_data: list) -> str:
        ...
        for agent_id in all_rounds_data[0]["contributions"].keys():
            confidences = [
                ...
                for round_data in all_rounds_data
                if agent_id in round_data["contributions"]
            ]
    ...
    def _generate_inter_agent_feedback(
        self, contributions: Dict[str, AgentContribution]
    ) -> Dict[str, Dict]:
        ...
        if devils_advocate_contribution and isinstance(
            ...
            critical_challenges = devils_advocate_contribution.content.get(
                "critical_challenges", []
            )
            alternative_recommendations = devils_advocate_contribution.content.get(
                "alternative_recommendations", []
            )
        ...
        for agent_id, contribution in contributions.items():
            ...
            if critical_insights:
                ...
                for critique_key, critique_value in critical_insights.items():
                    if agent_type.lower() in critique_key.lower() or any(
                        keyword in critique_key.lower()
                        for keyword in [agent_type.lower()[:4], agent_id.split("_")[0]]
                    ):
                        feedback["needs_refinement"] = True
                        feedback["devils_advocate_feedback"].append(
                            f"Critical analysis: {critique_value}"
                        )
                        feedback["suggestions"].append(
                            f"Address devil's advocate concern: {critique_value}"
                        )
                ...
                if isinstance(critical_challenges, list) and critical_challenges:
                    for challenge in critical_challenges[
                        :2
                    ]:  # Limit to top 2 challenges
                        if isinstance(challenge, str) and len(challenge) > 10:
                            feedback["suggestions"].append(
                                f"Consider critical challenge: {challenge}"
                            )
                            feedback["needs_refinement"] = True

                # Add alternative recommendations
                if (
                    isinstance(alternative_recommendations, list)
                    and alternative_recommendations
                ):
                    for alt_rec in alternative_recommendations[
                        :1
                    ]:  # Limit to 1 alternative
                        if isinstance(alt_rec, str) and len(alt_rec) > 10:
                            feedback["suggestions"].append(
                                f"Alternative approach suggested: {alt_rec}"
                            )
            ...
            if isinstance(contribution.content, dict):
                if "summary" not in contribution.content:
                    feedback["suggestions"].append(
                        "Add summary section for better integration"
                    )

                if len(contribution.content) < 3:
                    feedback["suggestions"].append(
                        "Expand content with more detailed analysis"
                    )
            ...
            if other_contributions:
                ...
                feedback["feedback_summary"] = (
                    f"Integrated feedback from {len(other_contributions)} agents: {', '.join(feedback_sources)}"
                )
            else:
                feedback["feedback_summary"] = (
                    "Initial contribution without cross-agent feedback"
                )
    ...
    def _track_collaboration_round(self, contributions: Dict[str, AgentContribution]):
        ...
        round_metadata = {
            ...
            "contribution_stats": {
                ...
                "average_confidence": sum(c.confidence for c in contributions.values())
                / len(contributions),
                "dependency_count": sum(
                    len(c.dependencies or []) for c in contributions.values()
                ),
            ...
            "quality_metrics": {
                "high_confidence_count": sum(
                    1 for c in contributions.values() if c.confidence > 0.9
                ),
                "medium_confidence_count": sum(
                    1 for c in contributions.values() if 0.7 <= c.confidence <= 0.9
                ),
                "low_confidence_count": sum(
                    1 for c in contributions.values() if c.confidence < 0.7
                ),
            },
        }
        ...
        privacy_policy = PrivacyPolicy(
            privacy_level=PrivacyLevel.INTERNAL, encryption_mode=EncryptionMode.NONE
        )
    ...
    def synthesize_final_artifact(self, query: str) -> AgentContribution:
        ...
        geo_insights_1 = (
            self.agent_contributions.get("geo_agent_001", {}).content
            if self.agent_contributions.get("geo_agent_001")
            else {}
        )
        geo_insights_2 = (
            self.agent_contributions.get("geo_agent_002", {}).content
            if self.agent_contributions.get("geo_agent_002")
            else {}
        )
        cultural_insights = (
            self.agent_contributions.get("culture_agent_001", {}).content
            if self.agent_contributions.get("culture_agent_001")
            else {}
        )
        logistics_insights = (
            self.agent_contributions.get("logistics_agent_001", {}).content
            if self.agent_contributions.get("logistics_agent_001")
            else {}
        )
        safety_insights = (
            self.agent_contributions.get("safety_agent_001", {}).content
            if self.agent_contributions.get("safety_agent_001")
            else {}
        )
        devils_advocate_insights = (
            self.agent_contributions.get("devils_advocate_001", {}).content
            if self.agent_contributions.get("devils_advocate_001")
            else {}
        )
        ...
        print(
            f"🔥 DEBUG: Logistics Agent insights: {len(str(logistics_insights))} chars"
        )
        print(f"🔥 DEBUG: Safety Agent insights: {len(str(safety_insights))} chars")
        print(
            f"🔥 DEBUG: Devil's Advocate insights: {len(str(devils_advocate_insights))} chars"
        )
        ...
        consensus_distance = geo_insights_1.get(
            "total_distance_km", 14550
        ) or geo_insights_2.get("total_distance_km", 14550)
        devils_advocate_summary = devils_advocate_insights.get(
            "summary", "No critical analysis available"
        )
        ...
        synthesized_artifact = {
            ...
            "consortium_response": {
                "executive_summary": {
                    ...
                    "critical_challenges_identified": critical_challenges[:5]
                    if critical_challenges
                    else ["No specific critical challenges identified"],
                },
                "comprehensive_contribution_summary": {
                    ...
                    "cultural_analysis": {
                        ...
                        "cultural_regions": cultural_insights.get(
                            "cultural_regions", []
                        ),
                        "meaningful_framework": cultural_insights.get(
                            "meaningful_framework", {}
                        ),
                        "cross_cultural_themes": cultural_insights.get(
                            "cross_cultural_themes", []
                        ),
                    },
                    "logistics_analysis": {
                        "agent_insights": logistics_insights,
                        "resource_requirements": logistics_insights.get(
                            "resource_requirements", {}
                        ),
                        "timeline_optimization": logistics_insights.get(
                            "timeline_optimization", {}
                        ),
                        "support_infrastructure": logistics_insights.get(
                            "support_infrastructure", {}
                        ),
                    },
                    "safety_analysis": {
                        ...
                        "emergency_procedures": safety_insights.get(
                            "emergency_procedures", []
                        ),
                    },
                    "critical_analysis": {
                        "devils_advocate_insights": devils_advocate_insights,
                        "agent_critiques": devils_advocate_insights.get(
                            "agent_critique", {}
                        ),
                        "critical_challenges": critical_challenges,
                        "alternative_recommendations": devils_advocate_insights.get(
                            "alternative_recommendations", []
                        ),
                    },
                },
                "multi_agent_analysis": {
                    "agent_consensus": {
                        ...
                        "consensus_confidence": sum(
                            c.confidence
                            for c in self.agent_contributions.values()
                            if c.agent_id != "devils_advocate_001"
                        )
                        / max(
                            1,
                            len(
                    ...
                    "devils_advocate_analysis": {
                        ...
                        "critical_confidence": self.agent_contributions.get(
                            "devils_advocate_001",
                            type("obj", (object,), {"confidence": 0.95}),
                        ).confidence,
                        "critical_summary": devils_advocate_summary,
                        "alternative_suggestions": devils_advocate_insights.get(
                            "alternative_recommendations", ["No alternatives provided"]
                        ),
                        "agent_critiques": devils_advocate_insights.get(
                            "agent_critique", {}
                        ),
                        "risk_assessment": devils_advocate_insights.get(
                ...
                "integrated_plan": {
                    "phase_1_departure": {
                        ...
                        "cultural_considerations": cultural_insights.get(
                            "cultural_regions", []
                        ),
                        "logistics_requirements": logistics_insights.get(
                            "resource_requirements", {}
                        ),
                        "safety_protocols": safety_insights.get("safety_protocols", {}),
                        "duration": logistics_insights.get(
                            "timeline_optimization", {}
                        ).get("total_estimated_duration", "45-60 days"),
                        "cultural_significance": "Sacred departure from ancestral lands with proper ceremonies",
                        "devils_advocate_concerns": devils_advocate_insights.get(
                            "critical_challenges", []
                        )[:2],
                    },
                    "phase_2_pacific_crossing": {
                        ...
                        if len(devils_advocate_insights.get("critical_challenges", []))
                        > 2
                        else [],
                    },
    ...
    def create_consortium_maif(
        self, final_artifact: AgentContribution, output_path: str
    ) -> bool:
        ...
        consortium_metadata = {
            "consortium_composition": {
                ...
                "specializations": [
                    contrib.agent_type for contrib in self.agent_contributions.values()
                ],
                "collaboration_model": "sequential_dependency_based",
                "synthesis_approach": "multi_perspective_integration",
            },
        ...
        self.maif.add_multimodal(
            consortium_metadata, title="Consortium Metadata", use_acam=True
        )
...
def generate_comprehensive_trace(
    agents: List[BaseAgent],
        ...
        if hasattr(agent, "maif") and hasattr(agent.maif, "decoder"):
            try:
                agent_profile["maif_state"] = {
                    "total_blocks": len(agent.maif.decoder.blocks)
                    if hasattr(agent.maif.decoder, "blocks")
                    else 0,
                    "embeddings_available": len(agent.maif.decoder.get_embeddings())
                    if hasattr(agent.maif.decoder, "get_embeddings")
                    else 0,
                    "version_history": len(
                        getattr(agent.maif.decoder, "version_history", {})
                    ),
                }
            except (AttributeError, TypeError, Exception) as e:
                logger.debug(
                    f"Could not access MAIF state for agent {agent.agent_id}: {e}"
                )
    ...
    for round_data in all_rounds_data:
        ...
        for agent_id, contribution in contributions_to_use.items():
            if hasattr(contribution, "content") and contribution.content:
                ...
                contrib_trace = {
                    "contribution_type": contribution.get(
                ...
                contrib_trace = {
                    ...
                    "confidence": 0.0,
                    "content_size": len(content_text),
                    "content_summary": str(contribution)[:500] + "..."
                    if len(str(contribution)) > 500
                    else str(contribution),
    ...
    for agent in agents:
        ...
        for i, refinement in enumerate(getattr(agent, "refinement_history", [])):
            ...
            evolution["evolution_metrics"]["confidence_changes"].append(
                refinement.get("confidence_change", 0)
            )
    ...
    for agent in agents:
        maif_info = {
            ...
            "version_tracking": bool(
                hasattr(agent.maif, "encoder")
                and hasattr(agent.maif.encoder, "version_history")
            ),
        }
    ...
    total_contributions = sum(
        len(getattr(agent, "contributions", [])) for agent in agents
    )
    ...
    trace["performance_metrics"] = {
        ...
        "average_confidence": sum(
            contrib.confidence for contrib in final_contributions.values()
        )
        / len(final_contributions)
        if final_contributions
        else 0,
        "collaboration_efficiency": total_contributions / len(agents) if agents else 0,
        "refinement_rate": total_refinements / total_contributions
        if total_contributions > 0
        else 0,
    }

    # Knowledge Graph (simplified)
    knowledge_graph = {"nodes": [], "edges": [], "concepts": set()}
    ...
    for agent_id, contribution in final_contributions.items():
        ...
        knowledge_graph["nodes"].append(
            {
                ...
                "type": "agent",
                "specialization": next(
                    (a.specialization for a in agents if a.agent_id == agent_id),
                    "unknown",
                ),
            }
        )
        ...
        knowledge_graph["nodes"].append(
            {
                ...
                "type": "contribution",
                "contribution_type": contribution.contribution_type
                if hasattr(contribution, "contribution_type")
                else "unknown",
            }
        )
        ...
        knowledge_graph["edges"].append(
            {"source": agent_id, "target": contrib_id, "relationship": "created"}
        )
    ...
    trace["final_state"] = {
        "final_contributions": {
            agent_id: {
                "contribution_type": contrib.contribution_type,
                "confidence": contrib.confidence,
                "content_summary": str(contrib.content)[:500] + "..."
                if len(str(contrib.content)) > 500
                else str(contrib.content),
            }
...
def demonstrate_multi_agent_consortium(
    num_rounds: int = 10, agent_counts: Dict[str, int] = None
):
    ...
    query = (
        "How do I walk from California to Nepal in a meaningful way - where I "
        "have infinite ability to swim, and don't need to sleep"
    )
    ...
    print(
        "PHASE 0: Initializing multi-agent consortium (geography, cultural, logistics, safety, devil's advocate) with enhanced tracking..."
    )
    print("-" * 60)

    print(
        "🔥 DEBUG: Creating multi-agent consortium with configurable specializations..."
    )
    ...
    for i in range(agent_counts["culture"]):
        ...
        print(
            f"  - {cultural_agent.agent_id}: Cultural insights and meaningful experiences"
        )
    ...
    for i in range(agent_counts["devils_advocate"]):
        ...
        print(
            f"  - {devils_advocate_agent.agent_id}: Critical analysis of ALL agent contributions"
        )
    ...
    print(
        f"PHASE 1: Collecting agent contributions with {num_rounds} collaboration rounds..."
    )
    print("-" * 60)
    ...
    for round_num in range(1, num_rounds + 1):
        ...
        current_contributions = coordinator_agent.collect_contributions(
            agents, query, enable_refinement=True
        )
        ...
        round_data = {"round_number": round_num, "contributions": {}}
        ...
        for agent_id, contribution in current_contributions.items():
            ...
            round_data["contributions"][agent_id] = {
                "confidence": contribution.confidence,
                "contribution_type": contribution.contribution_type,
                "content_summary": str(contribution.content)[:500] + "..."
                if len(str(contribution.content)) > 500
                else str(contribution.content),
            }
    ...
    llm_summary = coordinator_agent._generate_llm_summary_of_rounds(
    ...
    collaboration_history = getattr(
        coordinator_agent, "collaboration_history", all_rounds_data
    )
    ...
    final_contributions_trace = trace_dump.get("final_state", {}).get(
        "final_contributions", {}
    )
    ...
    for agent_id, contribution_data in final_contributions_trace.items():
        ...
        if refinement_history:
            ...
            for refinement in refinement_history[-2:]:  # Show last 2 refinements
                ...
                print(
                    f"       Confidence change: {refinement.get('confidence_change', 0.0):+.3f}"
                )
    ...
    refinement_feedback = {
        ...
        "suggestions": [
            ...
            "Include risk mitigation details",
        ],
        "positive": True,
    }

    refined_final_artifact = coordinator_agent.refine_contribution(
        final_artifact, refinement_feedback
    )
    coordinator_agent.add_contribution_to_maif(
        refined_final_artifact, update_existing=True
    )
    ...
    trace_dump = generate_comprehensive_trace(
        agents, coordinator_agent, collaboration_history, final_contributions
    )
    ...
    confidences = [
        contrib.get("confidence", 0.0) for contrib in final_contributions_trace.values()
    ]
    ...
    print(
        f"Total Distance: Extracted from {len(final_contributions_trace)} agent analyses"
    )
    print(
        f"Estimated Duration: Based on {performance_metrics.get('total_contributions', 0)} contributions"
    )
    print(
        f"Key Challenge: Identified through {performance_metrics.get('total_refinements', 0)} refinements"
    )
    print(
        f"Complexity: {len(agent_profiles)} specialized agents with {sum(p.get('iteration_count', 0) for p in agent_profiles.values())} total iterations"
    )
    print(
        f"Meaningful Framework: Multi-agent collaborative analysis with trace validation"
    )
    print(
        f"Consensus View: {performance_metrics.get('collaboration_efficiency', 0.0):.1f} collaboration efficiency score"
    )
    print(
        f"Critical Perspective: {critical_data.get('confidence', 0.0):.2f} confidence from critical analysis"
    )
    ...
    if logistics_data:
        ...
        if hasattr(final_artifact, "content") and isinstance(
            final_artifact.content, dict
        ):
            artifact_content = final_artifact.content
            if (
                "consortium_response" in artifact_content
                and "integrated_plan" in artifact_content["consortium_response"]
            ):
                integrated_plan = artifact_content["consortium_response"][
                    "integrated_plan"
                ]
        ...
        if hasattr(final_artifact, "content") and isinstance(
            ...
            if "consortium_metadata" in artifact_content:
                ...
                print(
                    f"  Contributing Agents: {len(consortium_meta.get('contributing_agents', []))}"
                )
                print(
                    f"  Synthesis Confidence: {consortium_meta.get('synthesis_confidence', final_artifact.confidence):.2f}"
                )
                print(
                    f"  Interdisciplinary Integration: {consortium_meta.get('interdisciplinary_integration', True)}"
                )
                print(
    ...
    collaboration_metrics = {
        ...
        "average_confidence": (
            sum(c.confidence for c in contributions.values()) / num_contributions
        )
        if num_contributions > 0
        else 0.0,
        "dependency_satisfaction": (
            sum(1 for c in contributions.values() if c.dependencies) / num_contributions
        )
        if num_contributions > 0
        else 0.0,
        "content_complexity": (
            sum(len(json.dumps(c.content)) for c in contributions.values())
            / num_contributions
        )
        if num_contributions > 0
        else 0.0,
    ...
    consortium_report = {
        ...
        "agents_participated": [
            agent.agent_id for agent in agents + [coordinator_agent]
        ],
        ...
        "quality_assessment": {
            "all_dependencies_satisfied": all(
                ...
                or all(
                    dep
                    in [contrib.contribution_type for contrib in contributions.values()]
                    for dep in c.dependencies
                )
                for c in contributions.values()
            ),
            "confidence_threshold_met": all(
                c.confidence >= 0.7 for c in contributions.values()
            ),
    ...
    print(
        f"  Quality Assessment: {consortium_report['quality_assessment']['content_completeness']}"
    )
    print(
        f"  Technical Features: {len(consortium_report['technical_features_demonstrated'])}"
    )
...
Examples:
  ...
  python multi_agent_consortium_demo.py --rounds 3 --geo-agents 1 --culture-agents 1 --logistics-agents 1 --safety-agents 1 --devils-advocate-agents 1  # Minimal setup
    ...
    for agent_type, count in agent_counts.items():
        ...
        if count > 10:
            print(
                f"Warning: Large number of {agent_type} agents ({count}) may impact performance"
            )
    ...
    if total_agents > 20:
        print(
            f"Warning: Large total number of agents ({total_agents}) may significantly impact performance"
        )
    ...
    try:
        ...
        result = demonstrate_multi_agent_consortium(
            num_rounds=args.rounds, agent_counts=agent_counts
        )
