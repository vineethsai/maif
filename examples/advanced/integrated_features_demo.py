...
def demonstrate_columnar_storage_features(workspace: Path):
    ...
    for column, column_stats in stats.items():
        if (
            isinstance(column_stats, dict)
            and "min_value" in column_stats
            and "max_value" in column_stats
        ):
            print(
                f"  - {column}: min={column_stats['min_value']}, max={column_stats['max_value']}, null_count={column_stats.get('null_count', 0)}"
            )
...
def demonstrate_version_management_features(workspace: Path):
    ...
    new_schema = Schema(
        ...
        fields=[
            ...
            SchemaField(
                name="tags", field_type="json", required=False, default_value=[]
            ),
            SchemaField(
                name="rating", field_type="float", required=False, default_value=0.0
            ),
        ],
...
def main():
    ...
    print(
        "- Columnar Storage: Efficient storage and retrieval with optimized compression"
    )
    print(
        "- Dynamic Version Management: Automatic file format updates and schema evolution"
    )
    print(
        "- Adaptation Rules Engine: Rules defining when/how MAIF can transition states"
    )
