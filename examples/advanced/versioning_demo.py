...
def demonstrate_versioning():
    ...
    block1 = encoder.add_text_block(original_text, metadata={
        "author": "Alice",
        "version": 1,
    ...
    block2 = encoder.add_text_block(updated_text, metadata={
        "author": "Bob",
        "version": 2,
    ...
    block3 = encoder.add_text_block(final_text, metadata={
        "author": "Alice",
        "version": 3,
...
def analyze_provenance(maif_path: str):
    ...
    for entry in provenance:
        ...
        ts_seconds = entry.timestamp / 1000000 if entry.timestamp > 1e12 else entry.timestamp
        try:
...
def perform_forensic_analysis(maif_path: str):
    ...
    print(f"   Risk Score: {report.get('risk_assessment', {}).get('risk_score', 0):.2f}")
