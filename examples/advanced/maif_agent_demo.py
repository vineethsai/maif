    ...
    async def run(self):
        ...
        for target in optimization_targets:
            ...
            try:
                # Monitor system
                perception = await self.perceive(
                    f"System metrics for {target['target']}: current value 0.65",
                    "text"
                )
    ...
    class LearningDemoAgent(MAIFAgent):
        async def run(self):
            ...
            for attempt in range(10):
                ...
                logger.info(f"Attempt {attempt + 1}: Success rate now {success_rate:.1%}")
...
async def demonstrate_memory_system():
    ...
    for i in range(5):
        perception = await agent.perceive(f"Memory test {i}: Important data point", "text")
