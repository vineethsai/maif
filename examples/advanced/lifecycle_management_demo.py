...
def create_sample_maifs(workspace: Path, num_files: int = 3) -> list:
    ...
    for i in range(num_files):
        ...
        encoder.add_binary_block(
            b"Binary data sample" * 100, metadata={"type": "test_data", "file_id": i}
        )
...
def demonstrate_merging():
    ...
    for strategy in strategies:
        ...
        stats = merger.merge(
            maif_paths, str(output_path), merge_strategy=strategy, deduplication=True
        )
...
def demonstrate_splitting():
    ...
    for i in range(50):
        encoder.add_binary_block(
            ...
            metadata={
                "block_id": i,
                "type": "binary",
                "subtype": "image" if i % 2 == 0 else "data",
            },
        )
    ...
    for strategy, kwargs in strategies:
        ...
        parts = splitter.split(
            str(maif_path), str(output_dir), split_strategy=strategy, **kwargs
        )
...
def demonstrate_self_governance():
    ...
    for i in range(200):
        encoder.add_text_block(
            f"Block {i}: " + "Content " * 100, {"block_id": i, "timestamp": time.time()}
        )
...
def demonstrate_lifecycle_manager():
    ...
    merge_stats = manager.merge_maifs(
        maif_paths[:3], str(merge_output), strategy="semantic"
    )
    ...
    parts = manager.split_maif(
        str(merge_output), str(split_output), strategy="count", blocks_per_file=20
    )
...
def demonstrate_advanced_governance():
    ...
    for event in report["history"]:
        print(
            f"  - {event['action']} at {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(event['timestamp']))}"
        )
