...
def synthesize_node(state: RAGState) -> RAGState:
    ...
    """
    print(
        f"✍️  [synthesize] Generating answer (iteration {state['iteration_count'] + 1})..."
    )
...
def build_revision_prompt(
    question: str, context: str, verification_results: dict
) -> str:
    ...
    if unverified:
        feedback.append(
            f"The following claims need better support: {', '.join(unverified)}"
        )
    if contradictions:
        feedback.append(
            f"The following claims contradict the sources: {', '.join(contradictions)}"
        )
