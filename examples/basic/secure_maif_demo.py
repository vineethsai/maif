...
def create_demo_file():
    ...
    for i, text in enumerate(texts):
        block_id = writer.add_text_block(
            text, 
            metadata={"source": f"document_{i}.txt", "language": "en"}
        )
    ...
    emb_id = writer.add_embeddings_block(
        embeddings,
        metadata={"model": "all-MiniLM-L6-v2", "source": "demo"}
    )
...
def verify_file(file_path: str):
    ...
    print(f"   Signed: {'✓' if info['is_signed'] else '✗'}")
    print(f"   Finalized: {'✓' if info['is_finalized'] else '✗'}")
    ...
    for i, block in enumerate(reader.get_blocks()):
        ...
        status = "✓" if not (block.header.flags & 0x20) else "⚠️ TAMPERED"
...
def demonstrate_tamper_detection(original_path: str):
    ...
    if is_valid:
        print("   ⚠️ Unexpected: file passed verification")
    else:
...
def show_embedded_metadata(file_path: str):
    ...
    print("\n   ℹ️  All metadata, provenance, and security info is embedded")
