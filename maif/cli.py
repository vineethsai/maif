    ...
    try:
        ...
        if input_file:
            ...
            with open(input_file, 'r') as f:
                content = f.read()
            block_id = encoder.add_text_block(content, metadata={'source': input_file, 'privacy_level': privacy_level})
            click.echo(f"Added text block from {input_file}: {block_id[:16]}...")
        
        # Add text content
        if text:
            for text_content in text:
                block_id = encoder.add_text_block(text_content, metadata={'privacy_level': privacy_level})
        ...
        if files:
            for file_path in files:
                ...
                if file_ext in ['.txt', '.md', '.json']:
                    block_id = encoder.add_text_block(data.decode('utf-8'), metadata={'source': file_path})
                else:
                    block_id = encoder.add_binary_block(data, BlockType.BINARY, metadata={'source': file_path})
...
@click.option('--permission', type=click.Choice(['read', 'write', 'admin']), required=True, help='Permission level')
...
@click.option('--manifest', help='Manifest file path (deprecated in v3)')
@click.option('--action', type=click.Choice(['status', 'audit', 'revoke']), required=True, help='Management action')
    ...
    try:
        ...
        if action == 'status':
            ...
            for entry in provenance[:10]:  # Show first 10
                click.echo(f"  • {entry.action} by {entry.agent_id} at {entry.timestamp}")
...
def create_maif(input_file, output, manifest, text, agent_id, compress, sign):
    ...
    try:
        ...
        if input_file:
            ...
            try:
                # Try text first
                encoder.add_text_block(data.decode('utf-8'), metadata={'source': input_file})
            except UnicodeDecodeError:
                encoder.add_binary_block(data, BlockType.BINARY, metadata={'source': input_file})
...
def extract_content(maif_file, manifest, output_dir, block_index):
    ...
    try:
        ...
        blocks_to_extract = [decoder.blocks[block_index]] if block_index is not None else decoder.blocks
