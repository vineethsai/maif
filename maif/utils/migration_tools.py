...
class VectorDBMigrator:
    ...
    def migrate_from_pinecone(
        ...
        try:
            ...
            logger.info(
                f"Starting migration from Pinecone index '{index_name}' ({total_vectors} vectors)"
            )
            ...
            logger.warning(
                "Pinecone migration requires manual ID management or data export"
            )
    ...
    def migrate_from_chroma(
        ...
        try:
            ...
            logger.info(
                f"Starting migration from ChromaDB collection '{collection_name}' ({total_docs} documents)"
            )
    ...
    def migrate_from_weaviate(
        ...
        try:
            ...
            logger.info(
                f"Starting migration from Weaviate class '{class_name}' ({total_docs} objects)"
            )
    ...
    def migrate_from_json(
        ...
        try:
            ...
            if isinstance(data, dict) and "documents" in data:
                ...
                raise ValueError(
                    "JSON must be a list of documents or dict with 'documents' key"
                )
            ...
            for doc in tqdm(documents, desc="Migrating from JSON"):
                try:
                    ...
                    if text_field in doc:
                        text_id = self.encoder.add_text_block(
                            doc[text_field], metadata=metadata
                        )
    ...
    def _add_vector_to_maif(
        self, vector_id: str, vector: List[float], metadata: Dict[str, Any], source: str
    ):
        ...
        self.encoder.add_embedding_block(
            ...
            metadata={"source_db": source, "original_id": vector_id, **metadata},
        )
...
def migrate_to_maif(source_type: str, target_path: str, **kwargs) -> MigrationResult:
    """
