...
class SimpleMAIFAgent:
    ...
    def remember(
        ...
        # Add to MAIF
        block_id = self.encoder.add_text_block(content_str, metadata=metadata)

        # Add embedding
        self.encoder.add_embedding_block(
            embedding.vector,
            block_id=f"{block_id}_embedding",
            metadata={"source_block": block_id, "model": embedding.model},
        )
    ...
    def recall(
        ...
        for memory_id, memory_data in self._memory_cache.items():
            ...
            similarity = self._cosine_similarity(
                query_embedding.vector, memory_data["embedding"].vector
            )
    ...
    def forget(
        ...
        # Note: Memories are removed from cache but MAIF files are immutable by design.
        # For permanent deletion, create a new artifact without the forgotten memories.

        logger.info(f"Forgot {forgotten_count} memories")
        return forgotten_count

    def explain_reasoning(
        self, decision: str, context_limit: int = 5
    ) -> Dict[str, Any]:
        """
        ...
        if relevant_memories:
            avg_relevance = sum(m["relevance"] for m in relevant_memories) / len(
                relevant_memories
            )
