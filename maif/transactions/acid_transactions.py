                ...
                while True:
                    try:
                        ...
                        if (
                            header_size < 0
                            or data_size < 0
                            or header_size > 1024 * 1024
                            or data_size > 100 * 1024 * 1024
                        ):
            ...
            try:
                ...
                while True:
                    ...
                    try:
                        ...
                        magic, size, seq_num, timestamp = struct.unpack(
                            "!QIIQ", header_data[:28]
                        )
                        ...
                        if (
                            entry_dict.get("operation_type") in ["begin", "write"]
                            and entry_dict.get("transaction_id")
                            not in self._committed_transactions
                        ):
                            keep_entry = True
                ...
                if len(self._committed_transactions) > 100:
                    self._committed_transactions = set(
                        list(self._committed_transactions)[-50:]
                    )
...
class MVCCVersionManager:
    ...
    def create_version(
        self, block_id: str, data: bytes, metadata: Dict, transaction_id: str
    ) -> int:
    ...
    def read_version(
        self, block_id: str, snapshot_version: Optional[int] = None
...
class ACIDTransactionManager:
    ...
    def begin_transaction(self) -> str:
        ...
        with self._lock:
            ...
            wal_entry = WALEntry(
                transaction_id=transaction_id, sequence_number=0, operation_type="begin"
            )
    ...
    def write_block(
        self, transaction_id: str, block_id: str, data: bytes, metadata: Dict
    ) -> bool:
    ...
    def abort_transaction(self, transaction_id: str) -> bool:
        ...
        with self._lock:
            ...
            wal_entry = WALEntry(
                transaction_id=transaction_id, sequence_number=0, operation_type="abort"
            )
            self.wal.write_entry(wal_entry)
...
class MAIFTransaction:
    ...
    def write_block(self, block_id: str, data: bytes, metadata: Dict) -> bool:
        ...
        return self.transaction_manager.write_block(
            self.transaction_id, block_id, data, metadata
        )
...
def create_acid_enabled_encoder(
    maif_path: str, acid_level: ACIDLevel = ACIDLevel.PERFORMANCE, agent_id: str = None
) -> "AcidMAIFEncoder":
...
class AcidMAIFEncoder:
    ...
    def __init__(
        self,
        maif_path: str = None,
        acid_level: ACIDLevel = ACIDLevel.FULL_ACID,
        agent_id: str = None,
    ):
        """
    ...
    def add_text_block(self, text: str, metadata: Optional[Dict] = None) -> str:
        ...
        if self._current_transaction:
            ...
            success = self._transaction_manager._write_block_direct(
                block_id, data, metadata
            )
    ...
    def add_binary_block(
        self, data: bytes, block_type: str, metadata: Dict = None
    ) -> str:
