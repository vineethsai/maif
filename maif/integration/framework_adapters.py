...
class MAIFLangChainVectorStore(BaseVectorStore):
    ...
    def __init__(
        ...
        from ..semantic.semantic_optimized import OptimizedSemanticEmbedder
        ...
        if self.maif_path.exists():
            try:
                ...
                self.encoder = MAIFEncoder(
                    str(self.maif_path), agent_id="langchain_adapter"
                )
                self.decoder = None
        else:
            self.encoder = MAIFEncoder(
        ...
        for block in self.decoder.blocks:
            if _is_text_block(block):
                ...
                self._document_cache[doc_id] = Document(
                    page_content=content, metadata=block.metadata or {}
                )
        ...
        **kwargs,
        ...
        for text, metadata, doc_id in zip(texts, metadatas, ids):
            ...
            if hasattr(self.embedder, "embed_texts"):
                ...
                embedding = (
                    embeddings[0].vector
                    if hasattr(embeddings[0], "vector")
                    else embeddings[0]
                )
            else:
    ...
    def similarity_search_with_score(
        ...
        if hasattr(self.embedder, "embed_texts"):
            ...
            query_embedding = (
                ...
                if hasattr(query_embeddings[0], "vector")
                else query_embeddings[0]
            )
        else:
...
class MAIFRetriever:
    ...
    def __init__(
        self,
        vectorstore: MAIFLangChainVectorStore,
...
class MAIFLlamaIndexVectorStore:
    ...
    def __init__(self, maif_path: str, dim: int = 384):
        ...
        if self.maif_path.exists():
            ...
            self.encoder = MAIFEncoder(
                str(self.maif_path), agent_id="llamaindex_adapter"
            )
            self.decoder = None
    ...
    def _load_nodes(self):
        ...
        for block in self.decoder.blocks:
            ...
            if (
                ...
                and block.metadata
                and block.metadata.get("node_id")
            ):
    ...
    def add(self, nodes: List[Any]) -> List[str]:
        ...
        for node in nodes:
            node_id = (
                ...
                or hashlib.md5(str(node).encode()).hexdigest()
            )
            ...
            if embedding is not None:
                self.encoder.add_embeddings_block(
                    [embedding], {"node_id": node_id, "dimensions": len(embedding)}
                )
                self._embeddings[node_id] = np.array(embedding)

            self._nodes[node_id] = {"id": node_id, "text": text, "metadata": metadata}
    ...
    def query(
        self, query_embedding: np.ndarray, similarity_top_k: int = 10, **kwargs
...
class MAIFMemGPTBackend:
    ...
    def __init__(
        self, maif_path: str, page_size: int = 4096, max_pages_in_memory: int = 10
    ):
        self.maif_path = Path(maif_path)
    ...
    def _load_pages(self):
        ...
        for block in self.decoder.blocks:
            block_type_name = (
                block.header.block_type.name
                if hasattr(block.header.block_type, "name")
                else str(block.header.block_type)
            )
            if block_type_name == "memory_page" or (
                block.metadata and block.metadata.get("type") == "memory_page"
            ):
    ...
    def read_page(self, page_id: int) -> Optional[Dict[str, Any]]:
        ...
        if self.decoder:
            for block in self.decoder.blocks:
                meta = block.metadata or {}
                if meta.get("type") == "memory_page" and meta.get("page_id") == page_id:
    ...
    def allocate_page(self) -> int:
        ...
        self.write_page(
            page_id,
            {"data": "", "metadata": {"created": time.time(), "modified": time.time()}},
        )
    ...
    def _evict_if_needed(self):
        ...
        while len(self._pages) > self.max_pages_in_memory:
            # Find LRU page
            lru_page_id = min(
                self._page_access_times.keys(), key=lambda k: self._page_access_times[k]
            )
    ...
    def _write_page_to_maif(self, page_id: int):
        ...
        self.encoder.add_binary_block(
            page_json.encode("utf-8"), BlockType.BINARY, metadata=metadata
        )
...
class MAIFSemanticKernelConnector:
    ...
    async def search_async(
        ...
        for doc, score in results:
            if (
                doc.metadata.get("collection") == collection
