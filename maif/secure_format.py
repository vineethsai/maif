...
class SecureFileHeader:
    ...
    def pack(self) -> bytes:
        ...
        agent_did_padded = self.agent_did[:64].ljust(64, b'\x00') if isinstance(self.agent_did, bytes) else self.agent_did.encode()[:64].ljust(64, b'\x00')
        sig_padded = self.file_signature[:256].ljust(256, b'\x00')
        reserved_padded = self.reserved[:4].ljust(4, b'\x00') if isinstance(self.reserved, bytes) else b'\x00' * 4
...
class SecureBlockHeader:
    ...
    def get_signable_data(self) -> bytes:
        ...
        signing_flags = self.flags & ~(BlockFlags.SIGNED | BlockFlags.IMMUTABLE | BlockFlags.VERIFIED | BlockFlags.TAMPERED)
        return struct.pack(
            '>IIIIQ16s32s32sI',
            self.size,
            self.block_type,
...
class FileFooter:
    ...
    def pack(self) -> bytes:
        return struct.pack('>4sQ32sI', self.magic, self.total_size, self.checksum, self.reserved)
    ...
    def unpack(cls, data: bytes) -> 'FileFooter':
        ...
        return cls(magic=unpacked[0], total_size=unpacked[1], checksum=unpacked[2], reserved=unpacked[3])
...
class SecureMAIFWriter:
    ...
    def _create_genesis_entry(self):
        ...
        entry = ProvenanceEntry(
            ...
            agent_did=self.agent_did,
            action="genesis",
            block_hash=hashlib.sha256(f"genesis:{self.agent_id}:{self.created}".encode()).hexdigest(),
            chain_position=0,
            metadata={
    ...
    def _add_provenance_entry(self, action: str, block_hash: str, metadata: Optional[Dict] = None):
    ...
    def finalize(self) -> str:
        ...
        with open(self.file_path, 'wb') as f:
            ...
            provenance_data = json.dumps([e.to_dict() for e in self.provenance]).encode()
            ...
            agent_did_bytes = file_header.agent_did if isinstance(file_header.agent_did, bytes) else file_header.agent_did.encode()[:64].ljust(64, b'\x00')
            ...
            for block in self.blocks:
                ...
                if block.header.metadata_size > 0:
                    meta_bytes = json.dumps(block.metadata or {}, sort_keys=True).encode()
                    f.write(meta_bytes)
                block_offsets.append((offset, block.header.size, block.header.block_type, block.header.flags))
        ...
        try:
            with open(self.file_path, 'rb') as f:
                ...
                for i in range(self.file_header.block_count):
                    ...
                    data_size = block_header.size - SecureBlockHeader.HEADER_SIZE - block_header.metadata_size
                    ...
                    self.blocks.append(SecureBlock(header=block_header, data=block_data, metadata=metadata))
                ...
                try:
                    ...
                    self.key_algorithm = self.security_info.get('key_algorithm', 'Ed25519')
                    pub_key_bytes = bytes.fromhex(self.security_info['public_key'])
                    self.public_key = ed25519.Ed25519PublicKey.from_public_bytes(pub_key_bytes)
                except:
                    self.security_info = {}
    ...
    def verify_integrity(self) -> Tuple[bool, List[str]]:
        ...
        for i, block in enumerate(self.blocks):
            if block.header.flags & BlockFlags.SIGNED:
                metadata_bytes = json.dumps(block.metadata or {}, sort_keys=True).encode()
                signable = block.header.get_signable_data() + block.data + metadata_bytes
                
                try:
                    raw_sig = self._extract_ed25519_signature(block.header.signature)
                    self.public_key.verify(raw_sig, signable)
                except InvalidSignature:
                    errors.append(f"Block {i} (ID: {block.header.block_id.hex()}) - TAMPERED: signature verification failed")
...
def create_secure_maif(
    ...
    if 'embeddings' in content:
        writer.add_embeddings_block(content['embeddings'], content.get('embeddings_metadata'))
    
    # Add binary blocks
    for binary in content.get('binaries', []):
        writer.add_binary_block(binary['data'], SecureBlockType.BINARY, binary.get('metadata'))
